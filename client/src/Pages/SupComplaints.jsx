...
const SupComplaints = () => {
  ...
  const fetchAppointments = async () => {
    ...
    try {
      const response = await axios.get(`${backendUrl}/admin/getallcomplaints`, {
        headers: {
  ...
  const handleMarkAsComplete = async (caseId) => {
    try {
      const token = localStorage.getItem("token");
      if (!token) {
      ...
      await axios.post(
        `${backendUrl}/admin/markcomplaintcompleted`,
      ...
      render: (_, record) => {
        ...
        return isCompleted ? null : (
          <Button
            ...
            onClick={() => handleMarkAsComplete(record.caseId)}
          >
            Close
          </Button>
        );
